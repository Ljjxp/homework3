...
public class MainActivity extends AppCompatActivity {
    ...
    private void resetTargetAnimation() {
        ...
        // 可以思考下，这里为什么要使用 ofArgb，而不是 ofInt 呢？(geyBackgroundColor返回值是rgb？)
        ...
        ObjectAnimator animator_scale = ObjectAnimator.ofFloat(target,"scaleX", 1, 2);
        animator_scale.setDuration(Integer.parseInt(durationSelector.getText().toString()));
        animator_scale.setRepeatCount(ObjectAnimator.INFINITE);
        animator_scale.setRepeatMode(ObjectAnimator.REVERSE);
        animator_scale.start();

        ObjectAnimator animator_scale2 = ObjectAnimator.ofFloat(target,"scaleY", 1, 2);
        animator_scale2.setDuration(Integer.parseInt(durationSelector.getText().toString()));
        animator_scale2.setRepeatCount(ObjectAnimator.INFINITE);
        animator_scale2.setRepeatMode(ObjectAnimator.REVERSE);
        animator_scale2.start();

        // TODO 2：在这里实现另一个 ObjectAnimator，对 target 控件的透明度进行修改，从 1 到 0.5f 循环
        ObjectAnimator animator_alpha = ObjectAnimator.ofFloat(target,"alpha", 1, 0.5f);
        ...
        animatorSet.playTogether(animator_alpha);
        animatorSet.playTogether(animator_scale);
        animatorSet.playTogether(animator_scale2);
        animatorSet.start();
    }
}
